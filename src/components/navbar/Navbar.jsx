      ...
      <div className="container-fluid d-flex justify-content-between align-items-center">
        ...
        <div>
          <form
            ...
            onSubmit={handleSearchSubmit} // Add form submission handler
          >
            <IoSearch className="searchicon cursor-pointer" />
            <input
              className="form-control bg-body-tertiary border-0 shadow-none "
              ...
              placeholder="Search"
              aria-label="Search"
              value={searchTerm} // Bind search term state to input value
              onChange={handleSearchChange} // Update search term on input change
            />
          </form>
          {searchResults.length > 0 && ( // Display search results if any
            <ItemSearchModal searchResults={searchResults} />
          )}
        </div>
        <div className="d-flex gap-3 align-items-center ms-0 ms-lg-4">
          <Link to="/cart" className="nav-link">
